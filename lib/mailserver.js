...
function saveAttachment (id, attachment) {
  ...
  let output = fs.createWriteStream(path.join(mailServer.mailDir, id, attachment.contentId))
...
function createMailDir () {
  if (fs.existsSync(mailServer.mailDir)) {
    return fs.existsSync(`${mailServer.mailDir}/${storeFile}`) ? loadStore() : saveStore()
  }
 ...
 */
function loadStore () {
  let rawData = fs.readFileSync(`${mailServer.mailDir}/${storeFile}`)
 ...
 */
function saveStore () {
  fs.writeFileSync(`${mailServer.mailDir}/${storeFile}`, JSON.stringify(store))
}
